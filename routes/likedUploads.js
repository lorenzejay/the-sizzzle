...
//like the post & if already like unsave the post
...
router.post("/", authorization, async (req, res) => {
  try {
    ...
    const query = await pool.query(
      "INSERT INTO liked_uploads(liked_by,upload_post) VALUES ($1, $2)",
      [user_id, upload_id]
    );
...
//check if loggedIn user already liked the post
...
router.get("/retrieve-liked-posts", authorization, async (req, res) => {
  try {
    const user_id = req.user;
    //get all posts from saved_by
    const query = await pool.query("SELECT * FROM liked_uploads WHERE saved_by = $1", [user_id]);
    const result = query.rows;
